...
class AzCliTokenCredentialsProvider : TokenCredentialsProvider {
    override suspend fun getCredentialsAsync(
        targetResource: String,
    ): KustoTokenCredentials {
        val azureCliCredential = AzureCliCredentialBuilder().build()
        val tokenRequestContext =
            TokenRequestContext().addScopes("$targetResource/.default")
        val token =
            azureCliCredential.getToken(tokenRequestContext).block()?.token
        val expiresOn =
            azureCliCredential
                .getToken(tokenRequestContext)
                .block()
                ?.expiresAt
        return KustoTokenCredentials(
            "JWT",
            token ?: throw Exception("Failed to acquire token"),
            expiresOn,
        )
    }
    ...
    override suspend fun getCredentialsAsync(
        ...
        val azureCliCredential =
            AzureCliCredentialBuilder().tenantId(tenantId).build()
        val tokenRequestContext =
            TokenRequestContext()
