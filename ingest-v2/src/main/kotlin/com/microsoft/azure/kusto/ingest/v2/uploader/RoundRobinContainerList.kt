 ...
 * ConfigurationCache (and thus the same RoundRobinContainerList instances)
 * will distribute their uploads evenly across containers.
 *
...
class RoundRobinContainerList(
    ...
    /**
     * Atomic counter for round-robin container selection.
     * Uses Long to handle high-frequency uploads without overflow concerns.
     * Initialized to -1 so the first getAndIncrement returns 0.
     */
    private val currentIndex = AtomicLong(-1)

    /**
     * Gets the next starting index for round-robin container selection.
     * Each call returns the next index in sequence, wrapping around.
     *
    ...
    fun getNextStartIndex(): Int {
        ...
        return Math.floorMod(currentIndex.incrementAndGet(), containers.size.toLong()).toInt()
    }

    /**
     * Creates a copy of the underlying container list.
     * This is useful when the list needs to be passed to APIs
     * that don't support RoundRobinContainerList.
     */
    ...
    companion object {
         ...
         */
        fun empty(): RoundRobinContainerList = RoundRobinContainerList(emptyList())

        /**
         * Creates a RoundRobinContainerList from a list of ExtendedContainerInfo.
         */
        fun of(containers: List<ExtendedContainerInfo>): RoundRobinContainerList =
            RoundRobinContainerList(containers)
    }
}