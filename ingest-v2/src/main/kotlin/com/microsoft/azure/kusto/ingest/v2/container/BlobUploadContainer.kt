...
class BlobUploadContainer(val configResponse: @NotNull ConfigurationResponse) : UploadContainerBase {

    // choose a random container from the configResponse.containerSettings.containers

    override suspend fun uploadAsync(name: String, stream: java.io.InputStream): String {
        ...
        val noUploadLocation = configResponse.containerSettings == null ||
                (configResponse.containerSettings.containers?.isEmpty() == true
                        && configResponse.containerSettings.lakeFolders?.isEmpty() == true)
        if (noUploadLocation) {
        ...
        val targetPath = if (configResponse.containerSettings.containers.isNullOrEmpty()) {
