...
class GzipCompressionStrategy : CompressionStrategy {
    ...
    private val logger: Logger = LoggerFactory.getLogger(GzipCompressionStrategy::class.java)

    override val compressionType: String = "gzip"

    companion object {
        /**
         * Default buffer size when estimated size is unknown.
         * 64KB is a good balance between memory usage and performance.
         */
         ...
         */
        val INSTANCE: GzipCompressionStrategy by lazy { GzipCompressionStrategy() }
    }
     ...
     */
    override suspend fun compress(input: InputStream, estimatedSize: Long): InputStream =
        withContext(Dispatchers.IO) {
            try {
                val initialBufferSize = calculateInitialBufferSize(estimatedSize)
                logger.debug(
                ...
                val byteArrayOutputStream = ByteArrayOutputStream(initialBufferSize)
                ...
                val compressionRatio = if (estimatedSize > 0) {
                    ((1.0 - compressedBytes.size.toDouble() / estimatedSize) * 100).coerceIn(0.0, 100.0)
                } else {
                    0.0
                }
                ...
                throw CompressionException("Failed to compress stream using GZIP", e)
            } catch (e: OutOfMemoryError) {
                logger.error("GZIP compression failed due to memory constraints: {}", e.message)
     ...
     *
     * We estimate compressed size as ~30% of original to avoid
     * excessive buffer resizing while not over-allocating.
     */
    private fun calculateInitialBufferSize(estimatedSize: Long): Int {
        return when {
            ...
            estimatedSize < DEFAULT_BUFFER_SIZE -> DEFAULT_BUFFER_SIZE
                ...
                minOf(estimatedCompressedSize, MAX_INITIAL_BUFFER_SIZE.toLong()).toInt()
            }
        }
    }
}
