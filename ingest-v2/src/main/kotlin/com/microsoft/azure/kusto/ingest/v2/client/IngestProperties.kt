...
value class ZipPattern(val regex: String) {
    init {
        ...
        kotlin.runCatching { regex.toRegex() }
            .onFailure { throw IllegalArgumentException("Invalid regex pattern: $regex", it) }
    }
}
...
sealed class IngestionMapping {
    ...
    data class Inline(val definition: String) : IngestionMapping() {
        init {
            require(definition.isNotBlank()) { "Mapping definition cannot be blank" }
        }
    }
}

/**
 * Represents the properties for data ingestion in Kusto using proper Kotlin constructs.
 */
data class IngestProperties(
    ...
    companion object {
        ...
        private object Keys {
            ...
            const val IGNORE_LAST_RECORD_IF_INVALID = "ignoreLastRecordIfInvalid"
    ...
    class Builder {
        ...
        fun deleteAfterDownload(delete: Boolean) = apply { this.deleteAfterDownload = delete }
        fun ignoreSizeLimit(ignore: Boolean) = apply { this.ignoreSizeLimit = ignore }
        fun ignoreFirstRecord(ignore: Boolean) = apply { this.ignoreFirstRecord = ignore }
        fun ignoreLastRecordIfInvalid(ignore: Boolean) = apply { this.ignoreLastRecordIfInvalid = ignore }
        fun extendSchema(extend: Boolean) = apply { this.extendSchema = extend }
        fun recreateSchema(recreate: Boolean) = apply { this.recreateSchema = recreate }
        
        fun mapping(mapping: IngestionMapping?) = apply { this.mapping = mapping }
        fun mappingReference(name: String) = apply { 
            this.mapping = if (name.isBlank()) null else IngestionMapping.Reference(MappingName(name))
        }
        fun mappingDefinition(definition: String) = apply { 
            this.mapping = if (definition.isBlank()) null else IngestionMapping.Inline(definition)
        }
        
        fun validationPolicy(policy: ValidationPolicy?) = apply { this.validationPolicy = policy }
        fun creationTime(time: Instant?) = apply { this.creationTime = time }
        fun zipPattern(pattern: String?) = apply { 
            this.zipPattern = if (pattern.isNullOrBlank()) null else ZipPattern(pattern)
        }
        
        fun addIngestByTag(tag: String) = apply { tags.ingestByTags.add(tag) }
        fun addDropByTag(tag: String) = apply { tags.dropByTags.add(tag) }
        fun addIngestIfNotExistsTag(tag: String) = apply { tags.ingestIfNotExistsTags.add(tag) }
        fun addAdditionalTag(tag: String) = apply { tags.additionalTags.add(tag) }
        
        fun addProperty(key: String, value: Any) = apply { additionalProperties[key] = value }
        
        fun build(): IngestProperties = IngestProperties(
     ...
     */
    val ingestByTags: Set<String> get() = tags.ingestByTags.toSet()
    val dropByTags: Set<String> get() = tags.dropByTags.toSet()
    val ingestIfNotExistsTags: Set<String> get() = tags.ingestIfNotExistsTags.toSet()
    val additionalTags: Set<String> get() = tags.additionalTags.toSet()
    
    
    /**
     * Returns a dictionary representation of the properties for API consumption.
     */
    internal fun toDictionary(format: DataFormat): Map<String, Any> {
        ...
        // Add mapping
        when (mapping) {
            is IngestionMapping.Reference -> result[Keys.MAPPING_REFERENCE] = mapping.name.value
            is IngestionMapping.Inline -> result[Keys.MAPPING] = mapping.definition
            null -> { /* no mapping */ }
        }
        ...
        if (ignoreLastRecordIfInvalid) result[Keys.IGNORE_LAST_RECORD_IF_INVALID] = true
    ...
    fun <T> tryGetProperty(key: String): T? {
        return when (key) {
            ...
            Keys.IGNORE_LAST_RECORD_IF_INVALID -> ignoreLastRecordIfInvalid as? T
 ...
 */
inline fun ingestProperties(configure: IngestProperties.Builder.() -> Unit): IngestProperties {
 ...
 * This centralizes the conversion logic and simplifies the API wrapper.
 */
internal fun IngestProperties.toApiRequestProperties(format: com.microsoft.azure.kusto.ingest.v2.models.Format): com.microsoft.azure.kusto.ingest.v2.models.IngestRequestProperties {
    return com.microsoft.azure.kusto.ingest.v2.models.IngestRequestProperties(
        ...
        ingestIfNotExists = tags.ingestIfNotExistsTags.toList().takeIf { it.isNotEmpty() },
        ...
        creationTime = creationTime?.let { java.time.OffsetDateTime.from(it.atOffset(java.time.OffsetDateTime.now().offset)) },
