...
object PathUtils {
    ...
    private val FORBIDDEN_CHARS = Pattern.compile("[^\\w-]", Pattern.CASE_INSENSITIVE)
    ...
    fun sanitizeFileName(baseName: String?, sourceId: String?): String {
        ...
        val baseNamePart = if (!base.isNullOrEmpty()) "_$fileNameSegment" else ""
        return sanitize(sourceId, TOTAL_TWO_SEGMENT_MAX_LENGTH - fileNameSegment.length) + baseNamePart
    }
    ...
    private fun sanitize(name: String?, maxSize: Int): String {
        ...
        if (sanitized.length > maxSize) {
            sanitized = sanitized.take(maxSize - TRUNCATION_SUFFIX.length) + TRUNCATION_SUFFIX
        }
        return sanitized
    }
    ...
    fun createFileNameForUpload(name: String): String {
        ...
        return PREFIX + "_$timestamp" + "_${UUID.randomUUID().toString().replace("-", "")}" + "_$name"
    }

    /**
     * Returns the base name of the file, with extensions and without any path. Works for both local paths and URLs.
     * Examples:
    ...
    fun getBasename(uri: String?): String? {
        ...
        if (uriObj == null || !uriObj.isAbsolute) {
            ...
            return uri.substringAfterLast('/', uri).substringAfterLast('\\', uri)
        }
