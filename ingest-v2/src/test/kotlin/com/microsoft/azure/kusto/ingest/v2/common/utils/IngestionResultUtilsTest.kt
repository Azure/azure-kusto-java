...
class IngestionResultUtilsTest {
    ...
    private fun createBlobStatus(status: BlobStatus.Status, sourceId: String = "test-id"): BlobStatus {
        return BlobStatus(
            sourceId = sourceId,
            status = status,
    ...
    fun `getSucceededResults returns only succeeded results`() {
        ...
        assertTrue(succeededResults.all { it.status == BlobStatus.Status.Succeeded })
    ...
    fun `test mixed status scenarios`() {
        ...
        assertEquals(1, IngestionResultUtils.getSucceededResults(allStatuses).size)
    }
}
