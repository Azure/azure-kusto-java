...
class KustoBaseApiClientTest {
    ...
    fun `getBearerToken retrieves token from TokenCredential`(): Unit =
        runBlocking {
            // Arrange
            val expectedToken = "test-access-token"
            val tokenCallCount = AtomicInteger(0)

            val mockTokenCredential =
            ...
            val client =
                TestableKustoBaseApiClient(
                    dmUrl =
                    "https://ingest-test.westus.kusto.windows.net",
            ...
            val trc =
                TokenRequestContext()
                    .addScopes(
                        "https://ingest-test.westus.kusto.windows.net/.default",
                    )
            ...
            assertEquals(
                1,
                tokenCallCount.get(),
                "getToken should be called exactly once",
            )
        }

    @Test
    @DisplayName(
        "getBearerToken should be called multiple times when token needs refresh",
    )
    fun `getBearerToken is called multiple times for token refresh`(): Unit =
        runBlocking {
            // Arrange
            ...
            assertEquals(
                2,
                tokenCallCount.get(),
                "getToken should be called twice for token refresh",
            )
        }
    ...
    fun `getBearerToken handles token credential errors`(): Unit = runBlocking {
        ...
        assertEquals(
            1,
            tokenCallCount.get(),
            "getToken should be called exactly once before failing",
        )
    }

    /**
     * Testable subclass that exposes the protected getBearerToken method for
     * testing.
     */
    private class TestableKustoBaseApiClient(
        ...
        suspend fun testGetBearerToken(
