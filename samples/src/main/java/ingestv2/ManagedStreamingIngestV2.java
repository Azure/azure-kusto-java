...
public class ManagedStreamingIngestV2 {
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println("This client automatically chooses between streaming and queued ingestion");
    ...
    static void ingestFromStream() throws Exception {
        ...
        ExtendedIngestResponse csvResponse = managedStreamingIngestClient.ingestAsync(csvStreamSource, csvProperties).get();
        ...
        ExtendedIngestResponse compressedResponse = managedStreamingIngestClient.ingestAsync(compressedStreamSource, csvProperties).get();
        ...
        ExtendedIngestResponse jsonResponse = managedStreamingIngestClient.ingestAsync(jsonStreamSource, jsonProperties).get();
    ...
    static void ingestFromFile() throws Exception {
        ...
        ExtendedIngestResponse csvResponse = managedStreamingIngestClient.ingestAsync(csvFileSource, csvProperties).get();
        ...
        ExtendedIngestResponse jsonResponse = managedStreamingIngestClient.ingestAsync(jsonFileSource, jsonProperties).get();
    ...
    static void demonstrateFallbackTracking() throws Exception {
        ...
        System.out.println("The ManagedStreamingIngestClient automatically falls back to queued ingestion");
        System.out.println("when data size exceeds the streaming limit (~4MB compressed).");
        System.out.println("Since data is automatically compressed, we use a larger dataset (~20MB)");
        ...
        System.out.println("Ingesting large dataset (" + formatBytes(dataBytes.length) + " uncompressed)...");
        System.out.println("(Watch for fallback log messages from ManagedStreamingIngestClient)");
        System.out.println();
        
        ExtendedIngestResponse response = managedStreamingIngestClient.ingestAsync(largeStreamSource, properties).get();
        ...
        if (response.getIngestionType() == IngestKind.QUEUED) {
            ...
            StatusResponse finalStatus = pollForCompletionManually(operation, Duration.ofSeconds(30), Duration.ofMinutes(2));
            printStatusResponse("Final Status", finalStatus);

        } else {
            System.out.println("NOTE: Data was ingested via STREAMING method.");
            System.out.println("This might happen if compression was very effective. Try increasing");
            System.out.println("the data size or using less compressible data patterns.");
        }
    }

    /**
     * Generates a large CSV dataset of approximately the target size.
     * The data follows the format expected by the sample table schema.
     * Uses varied data to reduce compression effectiveness.
     */
    private static String generateLargeCsvData(int targetSizeBytes) {
        ...
        // Format: int,guid,int,int,int,int,int,int,int,int,int,int,datetime,string,string,int,timespan,null,null
        while (sb.length() < targetSizeBytes) {
            // Use random values to reduce compression effectiveness
            sb.append(rowCount).append(",")
              .append(UUID.randomUUID()).append(",")  // Random GUID
              .append(random.nextInt(10000)).append(",")
              .append(random.nextInt(100000)).append(",")
              .append(random.nextLong()).append(",")
              .append(random.nextDouble() * 1000000).append(",")
              .append(random.nextInt()).append(",")
              .append(random.nextInt(1000)).append(",")
              .append(random.nextInt(5000)).append(",")
              .append(random.nextInt(10000)).append(",")
              .append(random.nextInt(100)).append(",")
              .append(random.nextInt(50)).append(",")
              .append("2024-").append(String.format("%02d", (rowCount % 12) + 1))
              .append("-").append(String.format("%02d", (rowCount % 28) + 1))
              .append("T").append(String.format("%02d", rowCount % 24))
              .append(":").append(String.format("%02d", rowCount % 60))
              .append(":").append(String.format("%02d", rowCount % 60))
              .append(".").append(String.format("%07d", random.nextInt(10000000)))
              .append("Z").append(",")
              .append("Row_").append(rowCount).append("_").append(random.nextInt(100000)).append(",")
              .append("\"Description with random data: ").append(random.nextLong())
              .append(" and more: ").append(UUID.randomUUID()).append("\"").append(",")
              .append(random.nextInt(100000)).append(",")
              .append(String.format("%02d:%02d:%02d", 
                      random.nextInt(24), random.nextInt(60), random.nextInt(60))).append(",")
              .append(",")
              .append("null")
              .append("\n");
            rowCount++;
    ...
    /**
     * Manually polls for completion by repeatedly calling getOperationDetailsAsync.
     * This demonstrates how to implement polling when the ManagedStreamingIngestClient
     * is used and queued fallback occurs.
     */
    private static StatusResponse pollForCompletionManually(
        ...
        while (System.currentTimeMillis() - startTime < timeoutMillis) {
            ...
            System.out.println("Still in progress... (In Progress: " + 
     ...
     */
    private static void printIngestionResult(String operationName, ExtendedIngestResponse response) {
        String ingestionMethod = response.getIngestionType() == IngestKind.STREAMING ? "STREAMING" : "QUEUED";
        System.out.println("[" + operationName + "] Ingestion completed using " + ingestionMethod + " method.");
        ...
        if (response.getIngestionType() == IngestKind.STREAMING) {
            ...
            System.out.println("  (High reliability - data will be available after batch processing)");
        }
        System.out.println();
    }
