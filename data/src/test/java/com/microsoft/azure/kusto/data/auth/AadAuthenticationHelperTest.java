...
public class AadAuthenticationHelperTest {
    ...
    void acquireWithClientCertificateNullKey()
            throws CertificateException, OperatorCreationException, PKCSException, IOException, URISyntaxException,
        ...
        ConnectionStringBuilder csb = ConnectionStringBuilder.createWithAadApplicationCertificate(
                "https://resource.uri", "client-id", x509Certificate, privateKey);

        MsalTokenProviderBase aadAuthenticationHelper =
                (MsalTokenProviderBase) TokenProviderFactory.createTokenProvider(csb);
        ...
        assertEquals(
                new HashSet<>(Collections.singletonList("https://kusto.kusto.windows.net/.default")),
                aadAuthenticationHelper.scopes);
    ...
    void useCachedTokenAndRefreshWhenNeeded()
            throws IOException, DataServiceException, URISyntaxException, CertificateException,
                    OperatorCreationException, PKCSException, DataClientException {
        String certFilePath = Paths.get("src", "test", "resources", "cert.cer").toString();
        String privateKeyPath = Paths.get("src", "test", "resources", "key.pem").toString();
        ...
        MsalTokenProviderBase aadAuthenticationHelperSpy =
                (MsalTokenProviderBase) spy(TokenProviderFactory.createTokenProvider(csb));
        ...
        assertEquals(
                new HashSet<>(Collections.singletonList("https://kusto.kusto.windows.net/.default")),
                aadAuthenticationHelperSpy.scopes);

        doReturn(authenticationResultFromRefresh)
                .when(aadAuthenticationHelperSpy)
        ...
        doReturn(authenticationResultNullRefreshTokenResult)
                .when(aadAuthenticationHelperSpy)
    ...
    void checkCloudSettingsAbnormal() throws URISyntaxException, DataServiceException, DataClientException {
        ...
        ConnectionStringBuilder csb =
                ConnectionStringBuilder.createWithUserPrompt("https://weird.resource.uri", "weird_auth_id", "");

        PublicAppTokenProviderBase aadAuthenticationHelper =
                (PublicAppTokenProviderBase) TokenProviderFactory.createTokenProvider(csb);
        ...
        assertEquals(
                "https://nostandard-login-input/weird_auth_id/", aadAuthenticationHelper.clientApplication.authority());
        ...
        SilentParameters silentParametersNormalUser =
                aadAuthenticationHelper.getSilentParameters(new HashSet<>(Collections.singletonList(new MockAccount(
                        "c0327b6e-814d-4194-8e7f-9fc7a1e5dea9.115d58c9-f699-44e0-8a53-e1861542e510", "", "", null))));
        ...
        SilentParameters silentParametersMsaUser =
                aadAuthenticationHelper.getSilentParameters(new HashSet<>(Collections.singletonList(new MockAccount(
                        "c0327b6e-814d-4194-8e7f-9fc7a1e5dea9.9188040d-6c67-4c5b-b112-36a304b66dad", "", "", null))));
    ...
    void checkCloudSettingsNormal() throws URISyntaxException, DataServiceException, DataClientException {
        ...
        ConnectionStringBuilder csb =
                ConnectionStringBuilder.createWithUserPrompt("https://normal.resource.uri", "auth_id", "");
        ...
        HashSet<String> scopes =
                new HashSet<>(Collections.singletonList(CloudInfo.DEFAULT_KUSTO_SERVICE_RESOURCE_ID + "/.default"));
        assertEquals(scopes, aadAuthenticationHelper.scopes);
