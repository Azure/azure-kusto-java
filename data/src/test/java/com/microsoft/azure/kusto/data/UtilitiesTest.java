...
class UtilitiesTest {
    ...
    void convertMillisToTimespan() {
        ...
        Assertions.assertEquals(
                timeout, clientRequestProperties.getOption(ClientRequestProperties.OPTION_SERVER_TIMEOUT));

        String serverTimeoutOptionStr = "01:40:02.1";
        long serverTimeoutOptionMillis =
                TimeUnit.HOURS.toMillis(1) + TimeUnit.MINUTES.toMillis(40) + TimeUnit.SECONDS.toMillis(2) + 100L;
        ...
        Assertions.assertEquals(ClientRequestProperties.MAX_TIMEOUT_MS, clientRequestProperties.getTimeoutInMilliSec());
    ...
    void createExceptionFromResponseNoResponse() {
        DataServiceException error =
                Utils.createExceptionFromResponse("https://sample.kusto.windows.net", null, new Exception(), "error");
    ...
    void createExceptionFromResponse404Error() {
        ...
        DataServiceException error = Utils.createExceptionFromResponse(
                "https://sample.kusto.windows.net", basicHttpResponse, new Exception(), "error");
    ...
    void createExceptionFromResponseOneApi() {
        ...
        DataServiceException error = Utils.createExceptionFromResponse(
                "https://sample.kusto.windows.net", basicHttpResponse, new Exception(), OneApiError);
        Assertions.assertEquals(
                "Query execution has exceeded the allowed limits (80DA0003): ., ActivityId='1234'", error.getMessage());
        Assertions.assertTrue(error.getCause() instanceof DataWebException);
        Assertions.assertTrue(error.isPermanent());
        Assertions.assertEquals(
                401, Objects.requireNonNull(error.getStatusCode()).intValue());
    }
    ...
    void createExceptionFromMessageError() {
        ...
        BasicHttpResponse basicHttpResponse = getBasicHttpResponse(401);
        DataServiceException error = Utils.createExceptionFromResponse(
                "https://sample.kusto.windows.net", basicHttpResponse, new Exception(), errorMessage);
    ...
    private BasicHttpResponse getBasicHttpResponse(int statusCode) {
        BasicHttpResponse basicHttpResponse =
                new BasicHttpResponse(new BasicStatusLine(new ProtocolVersion("http", 1, 1), statusCode, "Some Error"));
