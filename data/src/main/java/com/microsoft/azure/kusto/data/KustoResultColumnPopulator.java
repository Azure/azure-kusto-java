...
class KustoResultColumnPopulator<R, C, T extends KustoType<C>> {
    ...
    static <R, C, T extends KustoType<C>> KustoResultColumnPopulator<R, C, T> of(
            String name, int ordinal, T type, boolean isNullable, BiConsumer<R, C> valueSetter) {
        return new KustoResultColumnPopulator<>(name, ordinal, type, isNullable, valueSetter);
    }

    static <R, C, T extends KustoType<C>> KustoResultColumnPopulator<R, C, T> of(
            String name, T type, boolean isNullable, BiConsumer<R, C> valueSetter) {
        return new KustoResultColumnPopulator<>(name, UNSET_ORDINAL, type, isNullable, valueSetter);
    }

    static <R, C, T extends KustoType<C>> KustoResultColumnPopulator<R, C, T> of(
            int ordinal, T type, boolean isNullable, BiConsumer<R, C> valueSetter) {
        ...
        if (resultValue == null) {
            if (!this.isNullable) {
                throw new NullPointerException(
                        String.format("Column %s (ordinal %d) is not nullable", this.name, ordinal));
            }
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    String.format(
                            ...
                            this.name, ordinal, resultValue.getClass().toString(), this.type.clazz.toString()),
                    e);
        }
