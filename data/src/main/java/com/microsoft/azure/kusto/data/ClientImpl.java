...
public class ClientImpl implements Client, StreamingClient {
    ...
    public ClientImpl(ConnectionStringBuilder csb, CloseableHttpClient httpClient) throws URISyntaxException {
        ...
        if (host == null && auth.endsWith(FEDERATED_SECURITY_SUFFIX)) {
            csb.setClusterUrl(new URIBuilder()
                    .setScheme(clusterUrlForParsing.getScheme())
                    .setHost(auth.substring(
                            0, clusterUrlForParsing.getAuthority().indexOf(FEDERATED_SECURITY_SUFFIX)))
                    .toString());
        }
        ...
        aadAuthenticationHelper = clusterUrl.toLowerCase().startsWith(CloudInfo.LOCALHOST)
                ? null
                : TokenProviderFactory.createTokenProvider(csb);
    ...
    @Override
    public KustoOperationResult execute(String database, String command)
    ...
    public String executeToJsonResult(String database, String command, ClientRequestProperties properties)
        ...
        Map<String, String> headers =
                generateIngestAndCommandHeaders(properties, "KJC.execute", commandType.getActivityTypeSuffix());
        ...
        return Utils.post(
                ...
                jsonPayload,
                null,
                timeoutMs + CLIENT_SERVER_DELTA_IN_MILLISECS,
                headers,
                false);
    }
    ...
    public KustoOperationResult executeStreamingIngest(
        ...
        String clusterEndpoint =
                String.format(CommandType.STREAMING_INGEST.getEndpoint(), clusterUrl, database, table, streamFormat);
        ...
        Map<String, String> headers = generateIngestAndCommandHeaders(
                properties, "KJC.executeStreamingIngest", CommandType.STREAMING_INGEST.getActivityTypeSuffix());
        ...
        try {
            ...
            throw new DataClientException(
                    clusterEndpoint, "Error converting json response to KustoOperationResult:" + e.getMessage(), e);
        }
    }
    ...
    public InputStream executeStreamingQuery(String database, String command, ClientRequestProperties properties)
        ...
        Map<String, String> headers = generateIngestAndCommandHeaders(
                properties, "KJC.executeStreaming", commandType.getActivityTypeSuffix());
        addCommandHeaders(headers);
        String jsonPayload = generateCommandPayload(database, command, properties, clusterEndpoint);

        return Utils.postToStreamingOutput(
                httpClient, clusterEndpoint, jsonPayload, timeoutMs + CLIENT_SERVER_DELTA_IN_MILLISECS, headers);
    }
    ...
    private Map<String, String> generateIngestAndCommandHeaders(
            ClientRequestProperties properties, String clientRequestIdPrefix, String activityTypeSuffix)
            throws DataServiceException, DataClientException {
    ...
    private String generateCommandPayload(
            String database, String command, ClientRequestProperties properties, String clusterEndpoint)
            throws DataClientException {
        String jsonPayload;
        try {
            JSONObject json = new JSONObject().put("db", database).put("csl", command);
            ...
            throw new DataClientException(
                    ...
                    String.format(
                            clusterEndpoint,
                            "Error executing command '%s' in database '%s'. Setting up request payload failed.",
                            command,
                            database),
                    e);
        }
